...
public partial class Midnight_enemy : CharacterBody2D
{
	private int i = 1;
	...
	{
		//because the _PhysicsProcess function is ran every frame, count takes 30 frames
		//(half a second) to become 30
		Count += 1;
		//every half second, the position of the enemy will be increased, effectively teleporting
		// the enemy forward
		if (Count % 30 == 0)
		{
			Position = Position + (velocity * Velocity * Globe.enemy_speed * 100);
